...
def load_data(opt):
    ...
    # FM
    elif opt.dataset in ['FM']:
        transform = transforms.Compose([transforms.Resize(opt.isize),
                                        transforms.CenterCrop(opt.isize),
                                        transforms.ToTensor(),
                                        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)), ])

        train_ds = ImageFolder(os.path.join('/media/b227/jdddp/dataset/FM_img/Ankle_boot', 'train'), transform)
        valid_ds = ImageFolder(os.path.join('/media/b227/jdddp/dataset/FM_img/Ankle_boot', 'test'), transform)
    ...
    # FOLDER
        ...
        # train_ds = ImageFolder(os.path.join('/media/b220/jdddp/dataset/MeiLiu2', 'train'), transform)
        # valid_ds = ImageFolder(os.path.join('/media/b220/jdddp/dataset/MeiLiu2', 'test'), transform)
        train_ds = ImageFolder(os.path.join('/media/pikapika/jdddp/skip-ganomaly-master/MeiLiu2', 'train'), transform)
        valid_ds = ImageFolder(os.path.join('/media/pikapika/jdddp/skip-ganomaly-master/MeiLiu2', 'test'), transform)
